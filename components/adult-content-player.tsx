...
export default function AdultContentPlayer({ 
  title, 
  videoUrl, 
  videoUrl1, 
  videoUrl2,
  poster 
}: AdultContentPlayerProps) {
  const [isLoading, setIsLoading] = useState(true)
  const [activeSource, setActiveSource] = useState("player1")
  ...
  useEffect(() => {
    if (mp4Source && videoRef.current) {
      ...
      // Initialize Plyr with all controls
      playerRef.current = new Plyr(videoRef.current, {
        controls: [
          "play-large", // Big centered play button
          "play",
          "progress",
          "current-time",
          "mute",
